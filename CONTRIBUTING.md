# Contributing to aroundU Messenger iOS
...
Support can be found in [![aroundU Messenger iOS Matrix room #element-x-ios:matrix.org](https://img.shields.io/matrix/element-x-ios:matrix.org.svg?label=%23element-x-ios:matrix.org&logo=matrix&server_fqdn=matrix.org)](https://matrix.to/#/#element-x-ios:matrix.org).
...
The Xcode project can be directly compiled through the shared aroundU Messenger scheme which includes the main application as well as the unit and UI tests.
...
This will clone a copy of the SDK if needed, build it for all supported architectures and configure aroundU Messenger to use the built framework. To learn about additional options run
...
The project uses Localazy and is sharing its translations with the aroundU Messenger Android project: https://localazy.com/p/element
...
aroundU Messenger uses Fastlane for running actions on the CI and tries to keep the configuration confined to either [fastlane](fastlane/Fastfile) or [xcodegen](project.yml). 

Please run `bundle exec fastlane` to see available options.

### Network debugging proxy

It's possible to debug the app's network traffic with a proxy server by setting the `HTTPS_PROXY` environment variable in the aroundU Messenger scheme to the proxy's address (e.g. localhost:8080 for mitmproxy).
